...
def _resolve_font_settings(font_settings=None):
    """Return validated point sizes for preview and exported plots."""
    resolved = dict(DEFAULT_FONT_SETTINGS)
    if font_settings:
        for key in resolved:
            value = font_settings.get(key)
            if value is not None:
                try:
                    resolved[key] = max(5, min(48, int(value)))
...
def _style_axes(ax, x_label_angle: float = 0, font_settings=None):
    """Apply Arial typography using the current user-selected point sizes."""
    fonts = _resolve_font_settings(font_settings)
    ...
    ax.tick_params(
        ...
        width=0.45,
        length=4,
        labelsize=fonts["tick_label"],
    )

    for label in ax.get_xticklabels():
        label.set_fontfamily(FONT_FAMILY)
        label.set_fontweight("bold")
        label.set_fontsize(fonts["tick_label"])
    ...
    ax.xaxis.label.set_fontproperties(
        {"family": FONT_FAMILY, "weight": "bold", "size": fonts["axis_label"]}
    )
...
def draw_mass_plot(fig: Figure, plot_data: pd.DataFrame, font_settings=None):
    fonts = _resolve_font_settings(font_settings)
    fig.clear()
    ...
    for ax, sample in zip(axes, samples):
        ...
        ax.set_xlabel("Monoisotopic mass (Da)", fontdict=PLOT_FONT, fontsize=fonts["axis_label"])
        ax.set_ylabel("Relative abundance (%)", fontdict=PLOT_FONT, fontsize=fonts["axis_label"])
        ax.set_ylim(0, 105)
        ax.set_yticks([0, 25, 50, 75, 100])
        if sample is not None and count > 1:
            ax.set_title(str(sample), fontdict=PLOT_FONT, fontsize=fonts["title"])
        _style_axes(ax, font_settings=fonts)
    ...
    if handles:
        axes[0].legend(
            ...
            edgecolor="black",
            facecolor="white",
            prop={"family": FONT_FAMILY, "weight": "bold", "size": fonts["legend"]},
        )
    ...
    for ax, sample in zip(axes, samples):
        ...
        for index, species_class in enumerate(present):
            ...
            if species_class not in seen:
                ...
                labels.append(pc.class_to_label(species_class))
        ax.set_xlabel("Length (nt)", fontdict=PLOT_FONT, fontsize=fonts["axis_label"])
        ax.set_ylabel("Relative abundance (%)", fontdict=PLOT_FONT, fontsize=fonts["axis_label"])
        ax.set_ylim(0, 105)
        ax.set_yticks([0, 25, 50, 75, 100])
        ...
        if count > 1:
            ax.set_title(str(sample), fontdict=PLOT_FONT, fontsize=fonts["title"])
        _style_axes(ax, x_label_angle=x_label_angle, font_settings=fonts)
...
class MainWindow(QMainWindow):
    ...
    def _build_axis_export_group(self):
        ...
        font_note = QLabel(
            "Font sizes below are used in both the on-screen preview and saved TIFF/JPEG plots."
        )
        font_note.setWordWrap(True)
        font_note.setStyleSheet("color:#444;")
        form.addRow(font_note)

        self.axis_font_size_in = QSpinBox()
        self.axis_font_size_in.setRange(6, 32)
        self.axis_font_size_in.setValue(AXIS_LABEL_SIZE)
        self.axis_font_size_in.setSuffix(" pt")
        form.addRow("Axis-title font size:", self.axis_font_size_in)

        self.tick_font_size_in = QSpinBox()
        self.tick_font_size_in.setRange(6, 24)
        self.tick_font_size_in.setValue(TICK_LABEL_SIZE)
        self.tick_font_size_in.setSuffix(" pt")
        form.addRow("Tick-label font size:", self.tick_font_size_in)

        self.title_font_size_in = QSpinBox()
        self.title_font_size_in.setRange(6, 32)
        self.title_font_size_in.setValue(TITLE_SIZE)
        self.title_font_size_in.setSuffix(" pt")
        form.addRow("Panel-title font size:", self.title_font_size_in)

        self.legend_font_size_in = QSpinBox()
        self.legend_font_size_in.setRange(6, 24)
        self.legend_font_size_in.setValue(LEGEND_SIZE)
        self.legend_font_size_in.setSuffix(" pt")
        form.addRow("Legend font size:", self.legend_font_size_in)

        self.annotation_font_size_in = QSpinBox()
        self.annotation_font_size_in.setRange(6, 24)
        self.annotation_font_size_in.setValue(ANNOTATION_SIZE)
        self.annotation_font_size_in.setSuffix(" pt")
        form.addRow("Statistics/annotation size:", self.annotation_font_size_in)
    ...
    def _wire_signals(self):
        ...
        for font_control in (
            self.axis_font_size_in,
            self.tick_font_size_in,
            self.title_font_size_in,
            self.legend_font_size_in,
            self.annotation_font_size_in,
        ):
            font_control.valueChanged.connect(self._on_plot_style_changed)
    ...
    def _font_settings(self):
        """Font sizes used identically by preview and exported figures."""
        return {
            "axis_label": self.axis_font_size_in.value(),
            "tick_label": self.tick_font_size_in.value(),
            "title": self.title_font_size_in.value(),
            "legend": self.legend_font_size_in.value(),
            "annotation": self.annotation_font_size_in.value(),
        }

    def _on_plot_style_changed(self, *_):
        if self.results:
            self._update_plots()
    ...
    def _update_plots(self):
        ...
        draw_mass_plot(self.mass_fig, annotated, font_settings=font_settings)
    ...
    def _settings_dataframe(self):
        ...
        settings = {
            ...
            "AxisLabelFont": f"{FONT_FAMILY} Bold {self.axis_font_size_in.value()} pt",
            "TickLabelFont": f"{FONT_FAMILY} Bold {self.tick_font_size_in.value()} pt",
            "PanelTitleFont": f"{FONT_FAMILY} Bold {self.title_font_size_in.value()} pt",
            "LegendFont": f"{FONT_FAMILY} Bold {self.legend_font_size_in.value()} pt",
            "AnnotationFont": f"{FONT_FAMILY} Bold {self.annotation_font_size_in.value()} pt",
    ...
    def _build_publication_export_figures(self):
        ...
        draw_mass_plot(mass_figure, annotated, font_settings=font_settings)
