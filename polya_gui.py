...
class MainWindow(QMainWindow):
    ...
    def _build_axis_export_group(self):
        ...
        self.plot_format_in.addItem("TIFF (600 dpi)", "tiff")
        self.plot_format_in.addItem("JPEG (300 dpi)", "jpeg")
    ...
    def _save_figure(self, figure: Figure, path: str):
        ...
        if image_format == "jpeg":
            figure.savefig(
                ...
                bbox_inches="tight",
                facecolor="white",
                pil_kwargs={"quality": self.jpeg_quality_in.value()},
            )
        else:
            figure.savefig(path, dpi=600, format="tiff", bbox_inches="tight", facecolor="white")
    ...
    def on_export_plots(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            base = self._stem()
            figure_items = [
                (self.mass_fig, f"{base}_Mass_annotation_plot.{extension}"),
                (self.class_fig, f"{base}_Class_resolved_length_plot.{extension}"),
                (self.total_fig, f"{base}_Total_distribution_plot.{extension}"),
            ]
            paths = []
            for figure, filename in figure_items:
